...
# def hmm_cell_to_logo(hmm_cell):
    
#     logomaker_alphabet = ["A", "C", "D", "E", "F", "G", "H", "I", "K", "L", "M", "N", "P", "Q", "R", "S", "T", "V", "W", "Y"]
#     logomaker_perm = np.array([fasta.alphabet.index(aa) for aa in logomaker_alphabet], dtype=int)
...
#         emissions,
#         np.expand_dims(ut.background_distribution[:20], 0))
...
#     fig, ax = plt.subplots()
#     fig.set_size_inches(hmm_cell.length, 8)
...
#                   sequence=None, #optional single sequence which hidden sequence will be visualized
#                   title=""
#                  ):
#     spacing=1.0
#     path_color="#CC6600"
...
#         type_colors = ["#000000" if p > edge_show_threshold else "#E0E0E0" for i,p in zip(indices, type_probs)]
...
#             label += fasta.alphabet[sort[j]] + " (" + "%.2f" % B[i+1, sort[j]] + ")\n"
#         label += "en="+"%.2f" % probs["begin_to_match"][i]+"\n"
#         label += "ex="+"%.2f" % probs["match_to_end"][i]+"\n"
#         node_labels[i+1] = label
    
#     fig = plt.figure(figsize=(hmm_cell.length,9), frameon=False)
#     ax = fig.add_axes([0, 0, 1, 1])
#     p = tf.math.sigmoid(hmm_cell.init_flank)
#     ax.text(-4*spacing, 0, "Init flank, match: \n (%.2f, %.2f)" % (p, 1-p), fontsize=12)
...
#         ax.text(-2*spacing, 1.2*spacing, title, fontsize=30, color=path_color)
#         #ax.text(-2*spacing, -spacing/2, "Insertions", fontsize=20)
#         hidden_seq = viterbi(sequence, hmm_cell)
