...
def perturbate(cluster_indices, prob, num_clusters):
    """ 
    Args: 
            cluster_indices: A tensor of shape (k, b) that contains the cluster 
            index of each input sequence.
    ...
    P = tf.random.uniform(tf.shape(cluster_indices), 0, 1) < prob
    P = tf.cast(P, tf.int64)
    random_clusters = tf.random.uniform(
        tf.shape(cluster_indices), 0, num_clusters, dtype=tf.int64
    )
    cluster_indices = P * random_clusters + (1-P) * cluster_indices
