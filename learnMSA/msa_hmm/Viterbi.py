...
@tf.function
def viterbi_step(gamma_prev, emission_probs_i, transition_matrix):
...
def viterbi_chunk_dyn_prog(emission_probs, init, transition_matrix, local_gamma):    
    ...
    for i in tf.range(1, L):
        gamma_val = viterbi_step(tf.expand_dims(gamma_val, -2), emission_probs[:,:,i], transition_matrix)[...,0,:]
...
def viterbi_backtracking(gamma, transition_matrix_transposed, output_type=tf.int32):
...
def viterbi_full_chunk_backtracking(viterbi_chunk_borders, local_gamma, transition_matrix_transposed, output_type=tf.int32):
...
def viterbi(sequences, hmm_cell, end_hints=None, parallel_factor=1, return_variables=False):
    ...
    gamma = viterbi_dyn_prog(emission_probs, init, A, use_first_position_emission=parallel_factor==1)
