...
class TotalProbabilityCell(tf.keras.layers.Layer):
    ...
    def call(self, conditional_forward, states, training=None, init=False):
        """ Args:
            conditional_forward: A batch of conditional forward probabilities. Shape: (b, q*q)
        ...
        output = tf.math.log(scaled_forward) + loglik[:,tf.newaxis]
        return output, new_state
    
    def get_initial_state(self, batch_size=None, inputs=None, dtype=None):
        if self.reverse:
            init_dist = tf.ones((batch_size, self.cell.max_num_states), dtype=self.dtype)
